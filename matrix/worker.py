...
def multiply(data):
    """
    Funkcja mnożenia macierzy przez wektor w czystym Pythonie.
    data[0] to macierz A (lista list), data[1] to wektor X (lista list, gdzie każda wewnętrzna lista ma jeden element).
    Funkcja zwraca listę wyników y.
    """
    A = data[0]  # Macierz A
    X = data[1]  # Wektor X
    nrows = len(A)  # Liczba wierszy w macierzy
    ncols = len(A[0])  # Liczba kolumn w macierzy
    y = []  # Lista wyników

    # Mnożenie macierzy przez wektor
    for i in range(nrows):
        s = 0  # Inicjujemy sumę dla danego wiersza
        for c in range(ncols):
            s += A[i][c] * X[c]  # Dodajemy do sumy iloczyn elementu z macierzy i wektora
        y.append(s)  # Dodajemy wynik do listy
    return y
...
def worker_process(dataQueue, resultsQueue):
    ...
    while True:
        ...
        if task is None:
            break  # Jeśli nie ma zadań, kończymy pracę
        start_index, A_block, X = task  # Rozpakowujemy dane zadania
        # Obliczamy wynik mnożenia macierzy przez wektor
        block_result = multiply([A_block, X])
        resultsQueue.put((start_index, block_result))  # Wysyłamy wynik do kolejki wyników

def main():

    if len(sys.argv) < 2:
        print("Użycie: python worker.py <liczba_zadań>")
        sys.exit(1)
    try:
